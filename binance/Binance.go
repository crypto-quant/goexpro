...
func (bn *Binance) placeOrder(amount, price string, pair CurrencyPair, orderType, orderSide string) (*Order, error) {
	...
	cid := respmap["clientOrderId"].(string)
	return &Order{
		Cid:        cid,
		Currency:   pair,
		OrderID:    orderId,
...
func (bn *Binance) GetListenKey() (string, error) {
	params := url.Values{}
	path := bn.apiV3 + USER_DATA_STREAM_URI
	...
	return string(respmap["listenKey"]), nil
}
...
func (bn *Binance) KeepAliveListenKey(listenKey string) error {
	...
	path := bn.apiV3 + USER_DATA_STREAM_URI
	_, err := HttpPut(bn.httpClient, path, params, map[string]string{"X-MBX-APIKEY": bn.accessKey})
	if err != nil {
		return err
	}
	return nil
